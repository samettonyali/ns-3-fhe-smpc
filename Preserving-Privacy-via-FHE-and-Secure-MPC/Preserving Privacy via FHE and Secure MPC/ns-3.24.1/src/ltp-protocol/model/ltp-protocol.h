...
public:
   ...
   *
   * */
  void ReportStatus (uint32_t seqNum,
  ...
  typedef void (* SessionStatusTracedCallback) (uint32_t seqNum, 
    const StatusNotificationCode statusCode, std::vector<uint8_t> vectorOfBisiy,
    uint32_t bisiy1, bool bisiy2, uint64_t bisiy3, uint32_t bisiy4, Time timeStamp);
...
public:
   ...
   */
  uint32_t StartTransmission ( uint64_t sourceClientService, 
                               uint64_t dstClientService,uint64_t dstLtpEngine, 
                               std::vector<uint8_t> data, uint16_t rdSize, 
  ...
  void DirectSend (Ptr<Packet> packet, Ptr<LtpConvergenceLayerAdapter> cla);
   ...
   */
  void Receive (Ptr<Packet> packet, Ptr<LtpConvergenceLayerAdapter> cla, Address from);
   ...
   * \return  Local Engine ID.
   */
  void SetLocalEngineId (uint64_t engineID);
  /**
...
private:
   ...
   */
  void EncapsulateBlockData (uint64_t dstClientService, Ptr<SessionStateRecord> ssr, std::vector<uint8_t> data, uint16_t rdSize, uint64_t claimOffset = 0, uint64_t claimLength = 0, uint32_t claimSerialNum = 0, uint32_t seqNum = 0);
  
  Ptr<Packet> EncapsulateSegment (uint64_t dstClientService,  SessionId id, std::vector<uint8_t> data, uint64_t offset, uint64_t length, SegmentType type, uint32_t cpSerialNum, uint32_t rpSerialNum, u_int8_t fragmentType, uint8_t fragmentID, uint16_t fragmentSize, uint32_t seqNum);
 ...
 */
  ...
  typedef std::map<Address, uint32_t> TotalFragmentSizes;
  typedef std::map<Address, uint16_t> FromMeterMap;
  typedef std::map<uint16_t, uint32_t> MeterSeqNumMap;
  typedef std::map<std::pair<uint32_t, uint16_t>, uint16_t> TotalReceivedFragmentSizes;
  typedef std::map<std::pair<uint32_t, uint16_t>, std::vector<uint8_t> > ApplLDataMap;
  
  typedef std::map<std::pair<uint32_t, Address>, PacketRecord > PacketEntries;
  
  typedef std::map< Address, std::vector< uint8_t > > RemnantByteMap;
  
  RemnantByteMap m_remnantBytesMap;
  
  PacketEntries m_packetEntries;
  ...
  MeterSeqNumMap                m_meterToSeqNum;
  FromMeterMap                  m_fromToMeterMap;
  TotalReceivedFragmentSizes    m_receivedDataSizeMap;
  ApplLDataMap                  m_applDataMap;
  ...
  uint64_t m_dstEngineId;             //!< Local Engine ID
  ...
  uint32_t m_sSize;             //!< Retransmission cycle limit. RFC 5326 - section 6.22
