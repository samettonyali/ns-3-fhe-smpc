      ...
      if (m_state == ESTABLISHED || m_state == CLOSE_WAIT)
        { // Try to send the data out
          if (!m_sendPendingDataEvent.IsRunning ())
            {
              NS_LOG_LOGIC ("Scheduling a SendPendingData!!!");
...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT)
    { // send a SYN packet and change state into SYN_SENT
      ...
      NS_LOG_INFO ("Target IP Address: " << m_endPoint->GetPeerAddress());
      m_state = SYN_SENT;
    }
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  NS_LOG_LOGIC ("Socket " << this << " forward up " <<
                ...
                ":" << m_endPoint->GetLocalPort () << " Packet Size: " << packet->GetSize());
                            ...
                            const Address &toAddress)
{
  ...
  if (tcpHeader.GetFlags () & TcpHeader::ACK)
    {
      NS_LOG_LOGIC (m_endPoint->GetLocalAddress () << " received an ACK from " << m_endPoint->GetPeerAddress () << "!!!");
  ...
  if (m_endPoint != 0)
    {
      ...
      NS_LOG_LOGIC("The target IP address: " << m_endPoint->GetPeerAddress ());
    }
  else
    {
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if ( seq == m_txBuffer->HeadSequence () )
    {
      NS_LOG_LOGIC ("TcpSocketBase::SendDataPacket:: This is a retransmit!!!");
  ...
  while (m_txBuffer->SizeFromSequence (m_nextTxSequence))
    {
      ...
      NS_LOG_LOGIC ("Available Window Size: " << w);
      NS_LOG_LOGIC ("Segment Size: " << m_segmentSize);
      NS_LOG_LOGIC ("TX Buffer Size: " << m_txBuffer->Size());
      NS_LOG_LOGIC ("Size from sequence: " << m_txBuffer->SizeFromSequence (m_nextTxSequence));
...
TcpSocketBase::ReceivedData (Ptr<Packet> p, const TcpHeader& tcpHeader)
{
  ...
  if (expectedSeq < m_rxBuffer->NextRxSequence ())
    { // NextRxSeq advanced, we have something to send to the app
      if (!m_shutdownRecv)
        {
          NS_LOG_LOGIC ("The app will be notified!!!");
          NotifyDataRecv ();
        }
      // Handle exceptions
      if (m_closeNotified)
