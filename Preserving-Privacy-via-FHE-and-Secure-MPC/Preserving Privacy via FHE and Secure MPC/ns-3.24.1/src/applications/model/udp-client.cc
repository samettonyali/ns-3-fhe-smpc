...
UdpClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpClient")
    ...
    .AddAttribute ("Interval",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("FirstSendingTime",
                   "The time to wait between packets", TimeValue (Seconds (10.0)),
                   MakeTimeAccessor (&UdpClient::m_firstSendingTime),
                   MakeTimeChecker ())
...
UdpClient::StartApplication (void)
{
  ...
  m_sendEvent = Simulator::Schedule (m_firstSendingTime, &UdpClient::Send, this);
}
...
UdpClient::Send (void)
{
  ...
  //Service message: 0;   Connection request: 1;  Connection reply: 2
  SeqTsHeader seqTs;
//  seqTs.SetSeq (m_sent);
  seqTs.SetSeq ((uint32_t)0);
  ...
  if ((m_socket->Send (p)) >= 0)
    {
      ++m_sent;
      NS_LOG_INFO ("Packet #" << m_sent << " has been sent! TraceDelay TX " << m_size << " bytes to "
