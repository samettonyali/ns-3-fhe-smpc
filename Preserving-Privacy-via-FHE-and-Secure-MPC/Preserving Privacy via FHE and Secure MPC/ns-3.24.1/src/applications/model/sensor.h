...
public:
  ...
  Sensor ();
  Sensor (Ptr <LtpProtocol> protocol, Address address, bool isSender, 
...
private:
  ...
  void Construct (Ptr<Node> n,
                  ...
                  std::string tid,
                  const UniformRandomVariable& ontime,
                  const UniformRandomVariable& offtime,
                  uint32_t size);
  ...
  void StopSending ();
  void SendPacket ();
  void SendFragmentedPacket (uint32_t packetSize, uint32_t totalSize);
  ...
  uint16_t m_maxFragment;        // the maximum bytes for one segment before fragmentation
  Time            m_interval;
  Time            m_nextTime;
  Time            m_firstTime;
