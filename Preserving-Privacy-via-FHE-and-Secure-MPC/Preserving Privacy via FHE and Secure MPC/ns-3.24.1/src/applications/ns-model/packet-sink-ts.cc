...
PacketSinkTs::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PacketSinkTs")
    ...
    .AddAttribute ("DefaultRxSize", "The default size of packets received",
                   UintegerValue (512),
                   MakeUintegerAccessor (&PacketSinkTs::m_defSize),
                   MakeUintegerChecker<uint32_t> ())        
    .AddAttribute ("FileName", "The output filename",
                   StringValue ("roundstat"),
                   MakeStringAccessor (&PacketSinkTs::m_outputFilename),
                   MakeStringChecker ())   
...
void PacketSinkTs::HandleReport(Ptr<Packet> packet, Address from) 
{
   SeqTsHeader seqTs;
   uint32_t m_rxBytes;
...
bool PacketSinkTs::IsThereAnyPreviousStoredPacket(Address from) {
...
void PacketSinkTs::ReportStat (std::ostream & os)  
{
