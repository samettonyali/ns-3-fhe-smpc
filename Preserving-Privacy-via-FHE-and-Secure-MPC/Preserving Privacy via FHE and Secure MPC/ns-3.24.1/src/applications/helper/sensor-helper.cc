...
SensorHelper::SensorHelper (std::string protocol, Address address)
{
  m_factory.SetTypeId ("ns3::Sensor");
...
ApplicationContainer
SensorHelper::Install (std::string nodeName) const
{
...
int64_t
SensorHelper::AssignStreams (NodeContainer c, int64_t stream)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      for (uint32_t j = 0; j < node->GetNApplications (); j++)
        {
          Ptr<Sensor> onoff = DynamicCast<Sensor> (node->GetApplication (j));
          if (onoff)
            {
...
void 
SensorHelper::SetConstantRate (DataRate dataRate, uint32_t packetSize)
{
