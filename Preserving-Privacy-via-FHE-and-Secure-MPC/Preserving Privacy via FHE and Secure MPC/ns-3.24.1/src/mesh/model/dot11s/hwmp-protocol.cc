...
HwmpProtocol::ReceivePerr (std::vector<FailedDestination> destinations, Mac48Address from, uint32_t interface, Mac48Address fromMp)
{
  ...
  if (retval.size () == 0)
    {
      // Nico - 02/28/15
       if (!m_reportHwmpTcpInterfaceCallback.IsNull() )
          {
            NS_LOG_INFO("HWMP to Interface connected!");
            m_reportHwmpTcpInterfaceCallback(m_stats.initiatedPerr,Simulator::Now ());
          }  
      else {
          NS_LOG_INFO("HWMP to Interface is not connected!");
          }
      // Nico - 02/28/15
      return;
    }
...
uint16_t HwmpProtocol::ReportPathError ()
{
   return m_stats.GetPathErrorInfo ();
}
uint16_t HwmpProtocol::ReportPathErrorForward ()
{
   return m_stats.GetPathErrorInfoForward ();
}
uint16_t HwmpProtocol::ReportPathErrorLink ()
{
   return m_stats.GetPathErrorInfoLink ();
}
uint16_t HwmpProtocol::ReportPathErrorPath ()
{
   return m_stats.GetPathErrorInfoPath ();
}
uint16_t HwmpProtocol::Statistics::GetPathErrorInfo () {
  return initiatedPerr;
}
uint16_t HwmpProtocol::Statistics::GetPathErrorInfoForward () {
  return iPerrForward;
}
uint16_t HwmpProtocol::Statistics::GetPathErrorInfoLink () {
  return iPerrLink;
}
uint16_t HwmpProtocol::Statistics::GetPathErrorInfoPath () {
  return iPerrPath;
}
...
void
HwmpProtocol::SetReportHwmpTcpInterfaceCallback (Callback<void,uint32_t,Time> cb)
{
   m_reportHwmpTcpInterfaceCallback = cb;
}
