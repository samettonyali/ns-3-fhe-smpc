...
EdcaTxopN::DoDispose (void)
{
  ...
  NS_LOG_WARN ("Number of Transmissions: " << m_totTransmit << " Number of Retransmissions: " << m_totRetransmit );
}
  ...
  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      ...
      m_totTransmit++;
      NS_LOG_WARN ("Data Transmission Request!!!");
...
EdcaTxopN::MissedAck (void)
{
  ...
  if (!NeedDataRetransmission ())
    ...
    {
      NS_LOG_WARN ("Link Layer Retransmit " << m_ac << " " << m_currentHdr.GetAddr1 () << " " << m_currentHdr.GetAddr2 () << " " << m_currentHdr.GetAddr3 () );
      m_totRetransmit++;
...
void
EdcaTxopN::PrintNumberOfEdcaRetransmission (std::ostream & os) const
{
  NS_LOG_WARN(m_ac << " " << m_totRetransmit);
  os << m_ac << " " << m_totRetransmit << " " ;
}
